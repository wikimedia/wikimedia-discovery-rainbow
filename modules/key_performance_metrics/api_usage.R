output$kpi_api_usage_series <- renderDygraph({
  ...
  if ( input$kpi_api_usage_series_include_open ) {
    api_usage <- dplyr::mutate(api_usage, all = cirrus + ifelse(is.na(`cirrus (more like)`), 0, `cirrus (more like)`) + geo + language + open + prefix)
  } else {
    api_usage <- dplyr::mutate(api_usage, all = cirrus + ifelse(is.na(`cirrus (more like)`), 0, `cirrus (more like)`) + geo + language + prefix)
  }
  if ( input$kpi_api_usage_series_data == "raw" ) {
    ...
    return(dygraph(api_usage, main = "Calls over time", xlab = "Date",
             ...
             dyOptions(strokeWidth = 3, colors = RColorBrewer::brewer.pal(7, "Set2")[7:1],
  ...
  return(dygraph(api_usage_change, main = "Day-to-day % change over time", xlab = "Date", ylab = "% change") %>%
           dyLegend(width = 400, show = "always") %>%
           dyOptions(strokeWidth = 3, colors = RColorBrewer::brewer.pal(7, "Set2"),
